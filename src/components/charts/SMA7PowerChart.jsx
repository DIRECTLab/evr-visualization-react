...
const SMA7PowerChart = () => {
  ...
  const loadData = async () => {
    while ((currentPage) * pageSize < maxData) {
      const res = await api.ems.sma7.get({ params: { page: currentPage, pageSize: pageSize } })
      if (res.error) {
        setLoading(false)
      }
      if (currentPage === 0) {
  ...
  const loadNewData = async () => {
    ...
    const res = await api.ems.sma7.get({ params: { limit: 200, start: moment(newestPointDate).add(1, 'seconds').toISOString() } });
    if (res.error) {
      setLoading(false)
    }
